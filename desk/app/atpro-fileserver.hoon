::  atpro-fileserver: static Clay file-serving agent
::
...
++  on-load
  ...
  :~  ::  if the file root changed, set the new root up for tombstoning.
    ...
    ::
      ...
      ::  always clear prior cache entries.
      ::
      (turn ~(tap in cash.old) (curr store ~))
    ::
      ::  if the file root changed, remove tombstoning from the prior root.
      ::
      ?:  =(foot.old file-root)  ~
      [(set-norm [our q.byk]:bowl foot.old &)]~
    ::
      ::  Always rebind the web root on every on-load, like the api agent
      ::  does for /apps/atpro/api. Survives vere restarts and agent
